...
if __name__ == "__main__":
    if len(sys.argv) < 2:
        print("Usage: python ip_check.py <IP_ADDRESS> OR <FILENAME.txt>")
        sys.exit(1)

    user_input = sys.argv[1]

    # CHECK: Is the user input a file?
    if os.path.isfile(user_input):
        print(f"\n[*] Bulk Mode: Reading from {user_input}...")
        print("=" * 70)
        ...
        # If it's not a file, assume it's a single IP address
        print(f"\n[*] Single Mode: Checking {user_input}...")
        check_ip(user_input)
